...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === '/profile' && request.method === 'PATCH') { const body = await request.json<Json>(); const updates: Json = {}; if (typeof body.full_name === 'string') updates.full_name = body.full_name; if (typeof body.onboarding_completed === 'boolean') updates.onboarding_completed = body.onboarding_completed; if (typeof body.is_portfolio_public === 'boolean') updates.is_portfolio_public = body.is_portfolio_public; const rows = await supabase(env, `profiles?id=eq.${userId}`, { method: 'PATCH', headers: { Prefer: 'return=representation' }, body: JSON.stringify(updates) }) as Json[]; return json(rows[0], 200, origin) }
